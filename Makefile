.PHONY: all up logs down clean fclean build check-env re restart test test-unit test-health test-e2e prepare-socket
...
test-health:
	@podman compose exec backend npm run health > /dev/null 2>&1 && \
		...
		(printf "\033[41m\033[37m FAIL \033[0m Health check (GET /health)\n" && exit 1)
